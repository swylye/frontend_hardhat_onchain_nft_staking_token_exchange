    ...
    */
    const getNftStakedCount = async () => {
        try {
            ...
            for (let i = 0; i < _nftMintedCount; i++) {
                if (await stakingContract.isTokenStaked(i) == true) {
    ...
    */
    const getStakingRewardAmount = async () => {
        try {
            ...
            const provider = await getProviderOrSigner();
            const stakingContract = getStakingContractInstance(provider);
            const stakingRewardWei = await stakingContract.getRewardPerDay();
    ...
    const getAvailableToUnstake = async () => {
        try {
            const signer = await getProviderOrSigner(true);
            const stakingContract = getStakingContractInstance(signer);
            const stakedAmount = await stakingContract.getStakedAmount(await signer.getAddress());
            if (stakedAmount > 0) {
    ...
    const getAvailableToStake = async () => {
        try {
            ...
            for (let i = 0; i < tokenCount; i++) {
                ...
                if (await stakingContract.isTokenStaked(_variable) == false) {
                    unstakedCount += 1;
                }
            }
            if (unstakedCount == 0) {
