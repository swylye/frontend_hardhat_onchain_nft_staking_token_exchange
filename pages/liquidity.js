 ...
 */
    const removeAllLiquidity = async () => {
        try {
            const signer = await getProviderOrSigner(true);
            const exchangeContract = getExchangeContractInstance(signer);
            const removeLpAmountWei = await exchangeContract.balanceOf(await signer.getAddress())
            const remove_tx = await exchangeContract.removeLiquidity(removeLpAmountWei);
            setLoading(true);
            await remove_tx.wait();
            setLoading(false);
            window.alert("You have successfully removed all liquidity!");
            setRemoveLpAmount("0");
            await getBalances();
        } catch (err) {
            console.error(err);
                                ...
                                <TextField
                                    ...
                                    onChange={async (e) => {
                                        if (e.target.value > 0) {
                                            ...
                                            const _tokenAmountWei = await tokenAmountToAddPL(e.target.value)
                        ...
                        <div>
                            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                                ...
                                <TextField
                                    ...
                                    onChange={(e) => {
                                        if (e.target.value > 0) {
                                            setAddTokenAmountWei(utils.parseEther(e.target.value).toString())
                                        }
                                    }
                                    }
                                    defaultValue="0"
        ...
        return (
            ...
            >
                <div>
                    ...
                    <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                        <LoadingButton onClick={removeLiquidity} loading={loading} loadingIndicator="Loading.." variant="contained" width="fit-content" disabled={userLpBalance <= 0 ? true : false}>
                            Remove liquidity! 🔥
                        </LoadingButton>
                    </FormControl>
                    <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                        <LoadingButton onClick={removeAllLiquidity} loading={loading} loadingIndicator="Loading.." variant="contained" width="fit-content" disabled={userLpBalance <= 0 ? true : false}>
